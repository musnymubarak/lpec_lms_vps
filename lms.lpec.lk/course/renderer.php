...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function frontpage_categories_list() {
        ...
        $chelper->set_subcat_depth($CFG->maxcategorydepth)
            ...
            ->set_attributes(['class' => 'frontpage-category-cards']);

        $topcategory = core_course_category::top();
        $categories = $topcategory->get_children($chelper->get_categories_display_options());
        $totalcount = $topcategory->get_children_count();

        // View more link setup
        $viewmoreurl = $chelper->get_categories_display_option('viewmoreurl');
        $viewmoretext = $chelper->get_categories_display_option('viewmoretext', get_string('viewallsubcategories'));

        $content = '';

        if ($totalcount > 0) {
            // Card grid container
            $content .= html_writer::start_div('row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4');

            foreach ($categories as $category) {
                $content .= $this->render_category_card($category, $chelper);
            }

            $content .= html_writer::end_div(); // Close row

            // View more link
            if ($viewmoreurl && $totalcount > count($categories)) {
                $content .= html_writer::div(
                    html_writer::link($viewmoreurl, $viewmoretext, ['class' => 'btn btn-primary mt-4']),
                    'text-center'
                );
            }
        }
     ...
     */
    protected function render_category_card(core_course_category $category, coursecat_helper $chelper) {
        $content = '';
        $classes = 'col';
        $url = new moodle_url('/course/index.php', ['categoryid' => $category->id]);

        // Card container
        $content .= html_writer::start_tag('div', ['class' => $classes]);
        $content .= html_writer::start_div('card h-100 shadow-sm');
        ...
        $content .= html_writer::tag('h5', html_writer::link($url, $name, ['class' => 'card-title text-primary']));
       
        // Course count
        // if ($chelper->get_show_courses() === self::COURSECAT_SHOW_COURSES_COUNT) {
        //     $coursecount = $category->get_courses_count();
        //     $content .= html_writer::tag('p', get_string('courses') . ': ' . $coursecount, ['class' => 'card-text text-muted']);
        // }
       
        // Description (if available)
        if ($description = $chelper->get_category_formatted_description($category)) {
            $content .= html_writer::div($description, 'card-text small mt-2');
        }
       
        $content .= html_writer::end_div(); // End card-body
       
        // // Card footer
        // $content .= html_writer::start_div('card-footer bg-transparent border-top-0');
        // $content .= html_writer::link($url, get_string('browsecourses'), ['class' => 'btn btn-sm btn-outline-primary']);
        // $content .= html_writer::end_div(); // End card-footer
       
        $content .= html_writer::end_div(); // End card
        $content .= html_writer::end_tag('div'); // End col
